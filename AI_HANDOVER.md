...
English for French speakers (b1/b2 only). **Complete as of 10 June 2026** — all 71 chapters (b1: 20 grammaire + 3 redaction + 12 vocabulaire; b2: 18 grammaire + 5 redaction + 13 vocabulaire) have full content (slides, worksheet, game; flashcards + match for vocabulaire). Generated from the same-slug English-track chapters via `scripts/gen_fr_from_en.py` (drops printables links, prunes dead next-chapter links, pins shared-asset versions). English UI, consistent with the originally authored fr chapters.
...
**Rolled out 10 June 2026** to every eligible vocab chapter in a/, b/, c/ and fr/ (98 chapters; only the intentional applied-grammar chapters lack one). Generated by `scripts/gen_match.py` from the animals template; each hub has a Match activity card and sibling pages a Match nav button.
...
Generated by `builder.html` (generateMatch function) for one-offs, or `scripts/gen_match.py` in bulk. es/ and espanol-en/ vocab chapters don't have it yet (Spanish UI variant pending).
...
**Technical Debt:**
- ✅ **ES vocab stub:** resolved (merge-conflict markers in es/a2/vocabulary/health-and-body/flashcards.html; 78/78 now).
- ✅ **Code clutter (L8):** 23 one-off scripts archived to `scripts/legacy/` (10 June).
- **es/ vocab games:** 78 game.html files have placeholder items (M2 remnant).
